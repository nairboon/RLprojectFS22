...
deepqlearnerAgent = QLearnerAgent(optimizer='sgd', method="q-learning", activation="sigmoid",initialization="xavier",gamma=gamma,
                              eta=eta, epsilon_0=epsilon_0, beta=0.00005, N_h=[64], name=agent_2_lbl)


agents = [qlearnerAgent, sarsaAgent, deepqlearnerAgent]

arena_state = Arena(agents=agents,ce_state_check=True,ce_state_extra=True, ma_length=ma_length)
results_state = arena_state.sample(episodes=n_episodes, runs=n_runs)


arena_baseline = Arena(agents=agents, ma_length=ma_length)
...
plot_abs("avg_wins", f'Average Winrate per {ma_length} games')
plot_abs("avg_moves", f'Average number of moves {ma_length} per games')
...
plot_rel("avg_wins", f'Average Winrate per {ma_length} games')
plot_rel("avg_moves", f'Average number of moves {ma_length} per games')
